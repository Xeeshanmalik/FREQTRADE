#!/bin/bash
# dryrun_monitor.sh — read-only status probe for the UltraPrecision forward
# dry-run. Prints a compact status line plus any CHANGES since the last run
# (new closed trades, opened trades, watchlist shifts, bot-down / errors) and
# state in user_data/logs/monitor_state.json so repeated calls report deltas.
#
# Telegram: on a CHANGE (or when run with "force") it sends an alert via the
# Bot API, reading the token/chat_id from config.json at runtime (never
# hardcoded). Sending is stateless and does not conflict with freqtrade's own
# Telegram. Designed to run from host cron so monitoring survives any agent
# session ending.
#
# Usage: dryrun_monitor.sh [force]   # "force" => always send a Telegram summary
set -uo pipefail
...
python3 - "$STATE" "$SCORES" "$UP" "$ERRS" "$CONFIG" "$FORCE" <<'PY'
import json, sys, os, subprocess, datetime, urllib.request, urllib.parse
state_path, scores_path, up, errs, config_path, force = (
...
headline = f"[{now}] {status} | trades tot={tot} open={op} closed={closed} P/L={pnl:+.2f}% | watchlist={wl} | errs65m={errs}"
print(headline)
if changes:
    print("CHANGES:")
...
# --- Telegram alert on change or when forced ---
def telegram(msg):
    try:
...
if changes:
    telegram("🔔 dry-run monitor\n" + headline + "\n" + "\n".join("• "+c for c in changes))
elif force == "force":
    telegram("✅ dry-run heartbeat\n" + headline)
